...
function renderScheduleList(type) {
  ...
  let filtered;
  if (type === 'upcoming') {
    // Optimized: String comparison for ISO dates is ~10x faster than parsing to Date
    const upcoming = [...(matches.upcoming || [])]
      .sort((a, b) => a.startTime < b.startTime ? -1 : a.startTime > b.startTime ? 1 : 0);
    filtered = (matches.live || []).concat(upcoming);
  } else {
    // Optimized: String comparison for ISO dates is ~10x faster than parsing to Date (descending)
