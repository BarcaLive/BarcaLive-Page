...
function renderScheduleList(type) {
  ...
  let filtered;
  if (type === 'upcoming') {
    // ⚡ Bolt: Direct string comparison for ISO dates (asc) avoids ~2000 Date allocations per render
    const upcoming = [...(matches.upcoming || [])].sort((a, b) =>
      a.startTime < b.startTime ? -1 : a.startTime > b.startTime ? 1 : 0
    );
    filtered = (matches.live || []).concat(upcoming);
  } else {
    // ⚡ Bolt: Direct string comparison for ISO dates (desc) avoids ~2000 Date allocations per render
    filtered = [...(matches.finished || [])].sort((a, b) =>
      a.startTime > b.startTime ? -1 : a.startTime < b.startTime ? 1 : 0
    );
  }
